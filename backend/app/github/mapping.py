...
1. The CAM MVP project's "Status" field value, if the issue has one — run
   through the same free-text heuristic (app.services.status_mapping
   .classify_status_name), since a project board can have arbitrary column
   names (e.g. "Todo"/"In Progress"/"Done" or "Backlog"/"Doing"/"Shipped").
2. Otherwise, GitHub's own `state`/`stateReason` on the issue itself: CLOSED
   + stateReason=NOT_PLANNED -> canceled; CLOSED (any other reason,
   including the default COMPLETED) -> completed; OPEN -> unstarted.
"""
