...
* A single "issue" node in this schema already carries everything Linear
  ...
  caller) instead of a separate fetch_comments() call — GitHub's connection
  API makes that a non-issue (pun intended), unlike Zoho which has no bulk
  comment endpoint at all.
