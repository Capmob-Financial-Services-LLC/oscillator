...
class Actor(Base, TimestampMixin):
    """A Linear, Zoho Sprints, or GitHub user. Keyed on whichever source id is set."""
...
class Cycle(Base, TimestampMixin):
    ...
    id: Mapped[int] = mapped_column(BigInteger, Identity(), primary_key=True)
    # nullable: rows synced from Zoho Sprints (sprints) / GitHub (milestones)
    # carry zoho_id/github_id instead.
    linear_id: Mapped[str | None] = mapped_column(String(64), unique=True)
    zoho_id: Mapped[str | None] = mapped_column(String(64), unique=True)
    github_id: Mapped[str | None] = mapped_column(String(64), unique=True)
