"""Functional tests for the GitHub normalizer — label diffing
(upsert_github_issue_labels) and state resolution (upsert_github_issues),
mirroring test_linear_labels_normalizer.py's shape and coverage.
    ...
    *,
    ...
    if project_status is not None:
        project_items.append(
            {"project": {"title": "CAM MVP"}, "fieldValueByName": {"name": project_status}}
        )
    return {
        "id": f"issue-{number}",
        "number": number,
...
async def test_github_issue_state_resolution_and_label_diffing():
    ...
    async with Session() as session:
        ...
        assert n_issues == 1
        assert n_transitions == 1  # None -> backlog ("Todo" matches classify_status_name's backlog hints)
